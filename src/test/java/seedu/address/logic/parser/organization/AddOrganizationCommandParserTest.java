...
public class AddOrganizationCommandParserTest {
    ...
    private static final String VALID_CONTACT_NAME = "John Doe";
    ...
    private static final String CONTACT_DESC = " " + PREFIX_CONTACT_NAME + VALID_CONTACT_NAME;
    ...
    private static final String INVALID_CONTACT_DESC = " " + PREFIX_CONTACT_NAME + "1234"; // digits invalid
    ...
    public void parse_allFieldsPresent_success() {
        Organization expectedOrg = new OrganizationBuilder()
                ...
                .withContactName(VALID_CONTACT_NAME)
                .withPhone(VALID_PHONE)
                .withEmail(VALID_EMAIL)
                .build();

        assertParseSuccess(parser,
                ORG_DESC + CONTACT_DESC + PHONE_DESC + EMAIL_DESC,
    ...
    public void parse_missingCompulsoryField_failure() {
        ...
        assertParseFailure(parser,
                VALID_ORG_NAME + CONTACT_DESC + PHONE_DESC + EMAIL_DESC,
                expectedMessage);

        // missing contact name prefix
        assertParseFailure(parser,
                ORG_DESC + VALID_CONTACT_NAME + PHONE_DESC + EMAIL_DESC,
                expectedMessage);

        // missing phone prefix
        assertParseFailure(parser,
                ORG_DESC + CONTACT_DESC + VALID_PHONE + EMAIL_DESC,
                expectedMessage);

        // missing email prefix
        assertParseFailure(parser,
                ORG_DESC + CONTACT_DESC + PHONE_DESC + VALID_EMAIL,
                expectedMessage);

        // all missing
        assertParseFailure(parser,
                VALID_ORG_NAME + VALID_CONTACT_NAME + VALID_PHONE + VALID_EMAIL,
                expectedMessage);
    }
    ...
    public void parse_invalidValues_failure() {
        ...
        assertParseFailure(parser,
                INVALID_ORG_DESC + CONTACT_DESC + PHONE_DESC + EMAIL_DESC,
                OrganizationName.MESSAGE_CONSTRAINTS);

        // invalid contact name
        assertParseFailure(parser,
                ORG_DESC + INVALID_CONTACT_DESC + PHONE_DESC + EMAIL_DESC,
                OrganizationContactName.MESSAGE_CONSTRAINTS);

        // invalid phone
        assertParseFailure(parser,
                ORG_DESC + CONTACT_DESC + INVALID_PHONE_DESC + EMAIL_DESC,
                OrganizationPhone.MESSAGE_CONSTRAINTS);

        // invalid email
        assertParseFailure(parser,
                ORG_DESC + CONTACT_DESC + PHONE_DESC + INVALID_EMAIL_DESC,
                OrganizationEmail.MESSAGE_CONSTRAINTS);

        // multiple invalids — only first reported
        assertParseFailure(parser,
                INVALID_ORG_DESC + INVALID_CONTACT_DESC + INVALID_PHONE_DESC + INVALID_EMAIL_DESC,
                OrganizationName.MESSAGE_CONSTRAINTS);
    }

    @Test
    public void parse_duplicatePrefixes_failure() {
        String validArgs = ORG_DESC + CONTACT_DESC + PHONE_DESC + EMAIL_DESC;

        // multiple organization names
        assertParseFailure(parser,
                ORG_DESC + ORG_DESC + CONTACT_DESC + PHONE_DESC + EMAIL_DESC,
                Messages.getErrorMessageForDuplicatePrefixes(PREFIX_ORG));

        // multiple contact names
        assertParseFailure(parser,
                ORG_DESC + CONTACT_DESC + CONTACT_DESC + PHONE_DESC + EMAIL_DESC,
                Messages.getErrorMessageForDuplicatePrefixes(PREFIX_CONTACT_NAME));

        // multiple phones
        assertParseFailure(parser,
                ORG_DESC + CONTACT_DESC + PHONE_DESC + PHONE_DESC + EMAIL_DESC,
                Messages.getErrorMessageForDuplicatePrefixes(PREFIX_PHONE));

        // multiple emails
        assertParseFailure(parser,
                ORG_DESC + CONTACT_DESC + PHONE_DESC + EMAIL_DESC + EMAIL_DESC,
        ...
        assertParseFailure(parser,
                ...
                Messages.getErrorMessageForDuplicatePrefixes(
                        PREFIX_ORG, PREFIX_CONTACT_NAME, PREFIX_PHONE, PREFIX_EMAIL));
    }
}
