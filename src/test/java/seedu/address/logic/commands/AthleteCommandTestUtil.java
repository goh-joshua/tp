...
public class AthleteCommandTestUtil {
    ...
    public static void assertCommandSuccess(AthleteCommand command, AthleteModel actualModel,
        ...
        try {
            ...
            assertEquals(expectedModel.getAthleteBook(), actualModel.getAthleteBook());
    ...
    /**
     * Convenience wrapper to {@link #assertCommandSuccess(AthleteCommand, AthleteModel, CommandResult, AthleteModel)}
     * that takes a string {@code expectedMessage}.
     */
    public static void assertCommandSuccess(AthleteCommand command, AthleteModel actualModel, String expectedMessage,
                                            AthleteModel expectedModel) {
     ...
     */
    public static void assertCommandFailure(AthleteCommand command, AthleteModel actualModel, String expectedMessage) {
        // we are unable to defensively copy the model for comparison later, so we can
        // only do so by copying its components.
        AthleteBook expectedAddressBook = new AthleteBook(actualModel.getAthleteBook());
        List<Athlete> expectedFilteredList = new ArrayList<>(actualModel.getFilteredAthleteList());

        assertThrows(CommandException.class, expectedMessage, () -> command.execute(actualModel));
        assertEquals(expectedAddressBook, actualModel.getAthleteBook());
