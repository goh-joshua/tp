...
**`Main`** (consisting of classes [`Main`](https://github.com/AY2526S1-CS2103T-F13-3/tp/tree/master/src/main/java/seedu/address/Main.java) and [`MainApp`](https://github.com/AY2526S1-CS2103T-F13-3/tp/tree/master/src/main/java/seedu/address/MainApp.java)) is in charge of the app launch and shut down.
...
* stores the application data i.e., all `Person` objects (which are contained in a `UniquePersonList` object), as well as `Athlete`, `Organization`, and `Contract` objects.
...
**Note:** An alternative (arguably, a more OOP) model is given below. It has a `Tag` list in the application, which `Person` references. This allows the application to only require one `Tag` object per unique tag, instead of each `Person` needing their own `Tag` objects.<br>
